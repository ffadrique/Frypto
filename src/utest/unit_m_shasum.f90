...
! Synopsis  : Unit tests driver for m_shasum
!
...
! Initialise test suite
  ...
  suite = xfunit_suite( package=package, &
                        source='m_shasum.f03', &
                        annotation='Interface to file hasing based on SHA' )

! Create test
  ut = xfunit_unit( name='unit_m_shasum_test_001', &
                    classname='t_shasum', &
                    executer=unit_m_shasum_test_001, &
                    annotation='Single file hashing' )
  call suite%add_unit_test( ut )

! Create test
  ut = xfunit_unit( name='unit_m_shasum_test_002', &
                    classname='t_shasum', &
                    executer=unit_m_shasum_test_002, &
                    annotation='Multiple file hashing' )
  call suite%add_unit_test( ut )

! Create test
  ut = xfunit_unit( name='unit_m_shasum_test_003', &
                    classname='t_shasum', &
                    executer=unit_m_shasum_test_003, &
                    annotation='Multiple file hash checking' )
  call suite%add_unit_test( ut )
