...
! Synopsis  : Unit tests driver for m_uuid
!
...
! Initialise test suite
  suite = xfunit_suite( package=package, &
                        source='m_uuid.f03', &
                        annotation='Universally Unique Identifier' )

! Create test
  ut = xfunit_unit( name='unit_m_uuid_test_001', &
                    classname='t_uuid', &
                    executer=unit_m_uuid_test_001, &
                    annotation='Constructors' )
  call suite%add_unit_test( ut )

! Create test
  ut = xfunit_unit( name='unit_m_uuid_test_002', &
                    classname='t_uuid', &
                    executer=unit_m_uuid_test_002, &
                    annotation='Time/MAC' )
  call suite%add_unit_test( ut )

! Create test
  ut = xfunit_unit( name='unit_m_uuid_test_003', &
                    classname='t_uuid', &
                    executer=unit_m_uuid_test_003, &
                    annotation='Namespace' )
  call suite%add_unit_test( ut )

! Create test
  ut = xfunit_unit( name='unit_m_uuid_test_004', &
                    classname='t_uuid', &
                    executer=unit_m_uuid_test_004, &
