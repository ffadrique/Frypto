...
/*   printf( "    %12ld %12ld %12ld %12ld %12ld\n", a, b, c, d, y );
*/
   return y;
}
...
/*
  printf("-C");for(i=0;i<60;++i) printf( "-" ); printf("\n");
  k = 0;
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 256; j++) {
      printf( "%12ld", ctx->S[i][j] );
      if( ++k == 6 ) {
        k = 0; printf("\n");
      }
    }
...
/*    printf( "%16lX %16lX\n", datal, datar ); */
    ctx->P[i] = datal & 0xffffffff;
    ctx->P[i + 1] = datar & 0xffffffff;
  }
/*  printf( "\n" ); */
...
*/
  ...
  for (i = 0; i < 4; ++i) {
    for (j = 0; j < 256; j += 2) {
      blowfish_encrypt(ctx, &datal, &datar);
      ctx->S[i][j] = datal & 0xffffffff;
      ctx->S[i][j + 1] = datar & 0xffffffff;
    }
  }
