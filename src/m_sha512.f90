...
! Initial hash value
  integer(kind=8), dimension(8), parameter :: H0 = &
   [ int(z'6A09E667F3BCC908',8), int(z'BB67AE8584CAA73B',8), int(z'3C6EF372FE94F82B',8), int(z'A54FF53A5F1D36F1',8), &
     int(z'510E527FADE682D1',8), int(z'9B05688C2B3E6C1F',8), int(z'1F83D9ABFB41BD6B',8), int(z'5BE0CD19137E2179',8) ]

! SHA-512 processing constants
  integer(kind=8), dimension(80), parameter :: K = &
...
! Hashing algorithm for SHA-512
  type, extends(t_hash64) :: t_sha512
    private
...
!     Initialises the hashing base message-digest operation
      procedure, nopass :: initialise => sha512_initialise

!     Hashing update operation
!     Continues the message-digest operation, processing
!     another message block, and updating the context
      procedure :: update => sha512_update

!     Get hash size
      procedure, nopass :: get_hash_size => sha512_get_hash_size

  end type t_sha512

! Constructor interface
  interface sha512
    module procedure sha512_default
  end interface sha512
...
! Default constructor
pure function sha512_default() result(res)
...
end function sha512_default


! Initialisation
pure subroutine sha512_initialise( H )
...
end subroutine sha512_initialise


! Message disgst update
pure subroutine sha512_update( this, M, H )

! Calling object
  class(t_sha512), intent(in) :: this
...
! Local variables
  ...
  integer(kind=8), dimension(size_of_H) :: HH
...
end subroutine sha512_update


! Get the hash size (bytes) for this algorithm
pure function sha512_get_hash_size() result(res)
...
end function sha512_get_hash_size
